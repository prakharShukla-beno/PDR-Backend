...
const segmentController = {
  ...
  create: async (req, res, next) => {
    try {
      ...
      res.status(201).json({ success: true, message: "Segment create ho gaya", data: segment });
  ...
  getById: async (req, res, next) => {
    try {
      ...
      if (!segment) return res.status(404).json({ success: false, message: "Segment nahi mila" });
  ...
  update: async (req, res, next) => {
    try {
      ...
      res.status(200).json({ success: true, message: "Segment update ho gaya", data: segment });
    } catch (error) { next(error); }
  },
  ...
  delete: async (req, res, next) => {
    try {
      ...
      res.status(200).json({ success: true, message: "Segment delete ho gaya" });
    } catch (error) { next(error); }
  },
